    ...
    return (
        <Box>
            ...
            <Flex justify="space-around">
                {
                    cnyatr.map((c) => <Image src={c} alt="cmpny" />)
                }
            </Flex>
            </Box>
            <Flex align='center' p="40px" bg='#f8f8f8'>
                <Image width="500px" src="https://www.bitrix24.in/upload/optimizer/converted/images/content_en/screens/main/section_on_premise.jpg.webp?1663227423000" />
                <Stack spacing="25px">
                    <Text className={styles.txhd1}>Bitrix24 On-Premise Edition</Text>
                    {
                        stl.map((s) => <Flex align="center" gap={2}>
                            <AiFillCheckCircle color="#61d2f6" />
                            <Text className={styles.points}>{s}</Text>
                        </Flex>)
                    }
                    <Button borderRadius="20px" width="160px" color="#61d2f6" border="1px solid #61d2f6" variant="unstyled">LEARN MORE</Button>
                </Stack>
            </Flex>
